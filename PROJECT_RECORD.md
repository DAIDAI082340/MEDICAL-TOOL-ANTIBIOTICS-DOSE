...
- **v0.6.3 (2026-09-23)**:
  - **升級 App Logo 為「門診速查 100% 同款厚金屬浮雕勳章框（雙生系列款）」**：
    ...
    - 徹底解決前版「細金邊死貼切齊邊緣」導致在手機桌面上顯得平扁之問題，達成手機桌面多 App 間 100% 和諧一致之厚重金屬勳章高級質感。
    - 全面更新標準多尺寸圖示資產：
  ...
  - **Service Worker 快取版本升級 (`v0.6.3`)**：
    - `sw.js` 的 `CACHE_NAME` 升級為 `'antibiotics-dose-v0.6.3'`，自動觸發手機 PWA 與桌面瀏覽器清理舊圖示快取，立即生效呈現全新同款厚金邊圖標。
...
- **v0.5.9 (2026-09-18)**:
  - **表格列上下間距拉開（增加舒適呼吸感）**：
    ...
    - 表頭 `th` 同步拉開至 **`13px 18px`**（字體 `1.0rem`），類別列微調至 **`10px 18px`**，各行次不再緊貼壓迫。
  - **品項規格與途徑欄寬比例黃金平衡（68% : 32%）**：
    - 解決原先 `82% : 18%`（4.5:1）比例懸殊問題，將「抗生素品項：商品名（學名） 規格」與「途徑」微調為 **`68% : 32%`**（約 2.1:1）。
    - 右側途徑標籤適度向內收攏，左右均衡對稱，消弭大段無意義空白。
  - **手機版防跑版保護（「只要手機排版不要跑掉就好」）**：
    - 手機尺寸（390px 寬度）下，`68%`（~250px）完美容納長藥名，`32%`（~120px）完美置中擺放途徑標籤。
    - 手機版 `td` 專屬設定 `11px 12px`，手持觸控舒適，100% 不跑版、不產生橫向滾動。

- **v0.5.8 (2026-09-18)**:
  - **抗生素主表格精簡為 2 欄（品項 + 途徑）**：
    - 依使用者指示與截圖，主表格僅保留「**抗生素品項：商品名（學名） 規格**」（`82%`）與「**途徑**」（`18%`）兩大核心欄位。
    - 四大劑量分級（Stat, >50, 10-50, <10）與完整臨床備註全面交由下方卡片以大字體精準呈現，免除上方重複渲染。
    - 上方表格高度大幅釋放（單藥鎖定時僅高約 42px），騰出超過 250px 垂直可視空間，徹底消滅水平滾動條。
  - **手機版搜尋列恢復向下自然折行（告別往右滑）**：
    - 在行動端 `@media (max-width: 768px)` 中精確設定 `.search-panel { flex-wrap: wrap !important; }`。
    - 搜尋輸入框、商品名選單、學名選單、全部分類選單與途徑按鈕群全面支援自然向下堆疊（`flex: 1 1 100%`）。
    - 手機直式瀏覽完全回復歷史版本的順暢體驗，單手即可向下滾動操作，徹底告別水平向右滑動。
  - **下方臨床備註超長字數「完整顯示保證」**：
    - 針對 `Zavicefta` 等十餘行極長備註，於下方卡片臨床備註框配置自適應高度與 `max-height: 240px; overflow-y: auto;`。
    - 配合外層容器 `overflow-y: auto` 溢出保護，確保任何螢幕尺寸下所有備註文字 100% 完整可讀，絕不被視窗底緣截斷。
