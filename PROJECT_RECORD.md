...
- **v0.3.0 (2026-09-16)**:
  - **新增品項 - Daptomycin 500mg (Cubicin / 達托黴素)**：
    - 常規劑量 4–6 mg/kg QD（重症菌血症 6–8 mg/kg QD），洗腎病患 Q48H 給藥（每次 HD 透析後補充）。
    - 臨床監測：每 3–5 天定期追蹤 Blood Culture (B/C) 評估療效；每 7 天監測 CPK（防橫紋肌溶解/肌病變，CPK > 1000 有症狀或 > 2000 無症狀需停藥）。
    - 重要禁忌：會被肺表面活性劑 (Surfactant) 去活化，絕對不可用於治療肺炎。
  - **新增品項 - Xocova 125mg (Ensitrelvir / 恩賽特韋)**：
    - COVID-19 口服 3CL 蛋白酶抑制劑，發病 72 小時內投藥最佳。
    - 劑量：Day 1 給予 375mg (3# QD)，Day 2–5 給予 125mg (1# QD)，總療程 5 天。
    - 腎功能調整：輕中度不需調整，eGFR < 15 或洗腎病患不建議使用。
    - 警示：強效 CYP3A 抑制劑，禁忌併用多種藥物；孕婦禁用。
  - **搜尋與下拉選單雙向同步互動 (Bidirectional Sync)**：
    - 輸入關鍵字時：下方表格即時過濾高亮，上方精選卡片同步呈現第一筆相符藥物，下拉式選單自動同步定位該品項。
    - 下拉選單選取時：上方精選卡片同步呈現詳細劑量與備註，下方表格同步篩選聚焦該藥物，搜尋欄自動填入藥名。
    - 表格點選互動：點擊下方表格任一藥物列 (`tr.drug-row`)，上方精選卡片與下拉選單皆同步切換為該藥物並高亮邊框。
  - **下拉式選單英文字母 (A–Z) 排序**：將全品項在下拉清單中依英文名稱字母順序 A–Z 排列，查找更直覺便利。
  - **工具列強化**：新增「↺ 顯示全部」重置按鈕，以及單藥卡片「✕」關閉按鈕。
  - **正則表達式跳脫防護**：加入 `escapeRegExp` 輔助函式，避免括弧或斜線等特殊符號引發 RegExp 語法錯誤。
